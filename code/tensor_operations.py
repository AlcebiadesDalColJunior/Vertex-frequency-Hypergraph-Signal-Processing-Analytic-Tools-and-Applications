...
from scipy.fft import fft, fft2, fftn, ifft, ifft2
...
def degree_tensor(V,E,A=None):
    ...
    #######################################################################
    ## Replace this code snippet with the following for optimization of degree
    ## tensor calculation
    ...
    #######################################################################
    ## Option to optimize the calculation of the degree tensor
...
def t_eigendecomposition(A):
    ...
    for k in range(N3):
        ...
        if M == 4:
            ...
            eigenvalues_hat[:,:,k,k] = np.diag(eigenvalues_vec)
        if M == 5:
            eigenvalues_vec, eigenvectors_hat[:,:,k,k,k] = np.linalg.eig(A_hat[:,:,k,k,k])
            eigenvalues_hat[:,:,k,k,k] = np.diag(eigenvalues_vec)
...
def t_inverse(A):
    ...
    A_inverse_hat = np.zeros((N1,N1,N3), dtype=complex)
    
    for k in range(N3):
        A_inverse_hat[:,:,k] = np.linalg.inv(A_hat[:,:,k])
    
    A_inverse = ifft(A_inverse_hat)
...
def t_inverse4(A):
    ...
    A_inverse_hat_hat = np.zeros((N1,N1,N3,N4), dtype=complex)
    
    for k in range(N3):
        A_inverse_hat_hat[:,:,k,k] = np.linalg.inv(A_hat_hat[:,:,k,k])
    
    A_inverse_hat = ifft(A_inverse_hat_hat)
    A_inverse = ifft(A_inverse_hat)
