...
def hgrc(L, gamma=1, signal=None):
    """Compute the hypergraph regularization centrality for nodes.

    Returns
    -------
    nodes : dictionary
       Dictionary of nodes with hypergraph regularization centrality as the value.
    """
    
    n = L.shape[0]
    
    gr = hypergraph_regularization(L, gamma)
    ...
    I_centrality = {i+1:centrality[i] for i in range(n)}
